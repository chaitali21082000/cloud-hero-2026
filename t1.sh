...
# Run Python script with retry for intermittent 403 permission errors
log "Running Python client (with retry)"
export PROJECT_ID=$(gcloud config get-value core/project)
export GOOGLE_APPLICATION_CREDENTIALS="$PWD/key.json"

for attempt in {1..5}; do
  if python3 synchronous_doc_ai.py \
    --project_id="$PROJECT_ID" \
    --processor_id="$PROCESSOR_ID" \
    --location=us \
    --file_name=health-intake-form.pdf | tee results.txt; then
    log "Python script succeeded on attempt $attempt"
    break
  else
    exit_code=$?
    log "Python script failed on attempt $attempt (exit code $exit_code)."
    if [ $attempt -eq 5 ]; then
      log "Python script failed after 5 attempts. Capturing stderr:"
    ...
    log "Retrying in $((2**attempt)) seconds..."
    sleep $((2**attempt))
  fi
done
